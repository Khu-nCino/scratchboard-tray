...
export function watchAndSave(store: Store<AppState>) {
   watchStore(store, (state: AppState) => (
      state.settings.sfdxPath
   ), (value: string) => {
      electronStore.set('sfdxPath', value);
   });

   watchStore(store, (state: AppState) => (
      state.settings.theme
   ), (value: string) => {
      electronStore.set('theme', value);
   });
}

export function watchStore<T, V>(store: Store<T>, accessor: (state: T) => V, callback: (value: V) => void) {
